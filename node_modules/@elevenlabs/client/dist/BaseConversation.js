...
export class BaseConversation {
    ...
    async handleMCPToolCall(event) {
        ...
        if (this.status === "disconnecting" || this.status === "disconnected") {
            ...
            this.onError(`Ignored an approval request for MCP tool call ${toolCall.tool_call_id}, which arrived after the session ended`, this.mcpApprovalErrorContext(toolCall));
            return;
        }
        if (this.mcpApprovals.has(toolCall.tool_call_id)) {
            ...
            this.onError(`Ignored a repeated approval request for MCP tool call ${toolCall.tool_call_id}, which has already been handled`, this.mcpApprovalErrorContext(toolCall));
            return;
        }
        const approval = {
            state: "pending",
        ...
        try {
            ...
            this.onError(`MCP tool approval handler failed: ${e?.message}`, this.mcpApprovalErrorContext(toolCall));
            this.settleMCPApproval(toolCall, false);
            return;
        }
        if (typeof isApproved !== "boolean") {
            // Fail closed: only an explicit `true` may let a tool call through.
            this.onError(`MCP tool approval handler must resolve to a boolean, received ${typeof isApproved}`, this.mcpApprovalErrorContext(toolCall));
            this.settleMCPApproval(toolCall, false);
            return;
        }
     ...
     */
    settleMCPApproval(toolCall, isApproved) {
        const approval = this.mcpApprovals.get(toolCall.tool_call_id);
        if (!approval || approval.state !== "pending") {
            this.onError(`Discarded an approval decision for MCP tool call ${toolCall.tool_call_id}, which is no longer awaiting approval`, this.mcpApprovalErrorContext(toolCall));
            return;
        }
        approval.state = "settled";
        this.sendMCPToolApprovalResult(toolCall.tool_call_id, isApproved);
    }
    markMCPApprovalStale(toolCallId) {
    ...
    mcpApprovalErrorContext(toolCall) {
        return {
    ...
    handleExternalAgentDisconnected(_event) {
        ...
        switch (parsedEvent.type) {
            ...
            case "mcp_tool_call": {
                try {
                    ...
                    this.onError(`Unexpected error in MCP tool call handling: ${error instanceof Error ? error.message : String(error)}`, {
